...
### Number of Engaged Pins — Rule of Thumb $$N_{eff} \approx \frac{N_{p}}{4}$$ Derivation: - Half of all pins lie in the load-bearing zone ($\alpha < 90°$). Of those, only $\approx 50\%$ carry significant load due to manufacturing tolerances $$N_{eff} = \tfrac{N_{p}}{2} \cdot \tfrac{1}{2} = \frac{N_{p}}{4}$$
