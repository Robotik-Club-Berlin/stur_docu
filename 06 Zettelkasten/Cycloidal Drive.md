...
![[cycloid-drive.jpg]]
## Transmission ratio
With $N_{l}$ being the number of lobes on the cycloidal disk and $N_{p}$ being the number of pins in the outer ring, the transmission ratio is defined as $$i = \frac{N_{l}}{N_{p} - N_{l}}$$ In most cases $n_p = n_l + 1$ which will result in a transmission of $n_{l}:1$
...
Conservative estimate including a peak factor of 2 for uneven load distribution among engaged pins: $$F_{p,max} = \frac{2 \cdot T_{out}}{N_{eff} \cdot R_p \cdot \cos(\alpha_{min})}$$where $\alpha_{min}$ is the minimum pressure angle within the load-bearing region (read from the $\alpha(\theta)$ curve).
### Maximum output shaft Pin force
$$F_{out,max} = \frac{4\, T_{out}}{N_{out} \cdot R_{out}}$$
...
With safety factor K (often 2-3):$$F_{bearing,max} = K \cdot \frac{T_{out}}{i \cdot e}$$